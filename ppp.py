...
def show_login_page():
    ...
    # Check if login has been successful by checking session_state
    if "logged_in" not in st.session_state:
        st.session_state.logged_in = False

    if st.button("Login") and not st.session_state.logged_in:  # Only proceed if not logged in already
        if not Batch_No:
